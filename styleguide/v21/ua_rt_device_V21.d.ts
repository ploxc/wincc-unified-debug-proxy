...
interface HMIAuditSysFct {
   ...
   * @returns Promise
   */
  InsertElectronicRecord(ObjectName: string, Category: string, OperationType: hmiOperationType, OldValue: any, NewValue: any, ConfirmationType: hmiConfirmationType, Reason?: ( HMITextList | string )): Promise<{}>;
  /**
...
interface HMIDataSet {
  ...
  RemoveAll(): void;
  /**
   * Triggers an update of the dataset, causing bound screen items to refresh (V20+).
   */
  triggerDataSet(): void;
}
...
interface HMIRuntime {
  ...
  /**
   * Writes a string to the trace buffer (V19+). Enhanced trace with additional context.
   *
   * @param message the string message for the trace buffer
   */
  Trace2(message: string): void;
  /**
   ...
   * @returns String
   */
  GetDetailedErrorDescription(errorCode: number): string;
  /**
   * Communication interface for system functions (V19+).
   */
  Communication: HMICommunication;
  /**
   * User management interface for import/export operations (V21+).
   */
  UserManagement: HMIUserManagement;
}
...
interface HmiUIEnums_HmiRequestResult{
   ...
   * InvalidName
   *
   * Request was rejected because the name is invalid
   */
  InvalidName: 3;
}
...
interface HmiUIEnums_HmiWebControlID{
   ...
   * Previous
   *
   * Operation, currently no technical concept for WebRH
   */
  Previous: 2;
  /**
   * Next
...
interface HmiFunctionTrendControl {
  ...
  FireCommand(commandId: number, custom: boolean): void;
  /**
   * Resets all runtime customizations (column order, widths, visibility, etc.) back to the configured defaults (V21+).
   */
...
interface HMIScreenItemInterface extends HmiScreenItemBase {
  ...
  CheckAuthorization(): boolean;
  /**
   * Returns the bounding box of the screen item in screen coordinates (V20+).
   *
   * @returns Object with Left, Top, Width, Height properties in screen coordinates
   */
  GetBoundingBoxInScreenCoordinates(): { Left: number; Top: number; Width: number; Height: number };
}
...
/**
 * Data object received in tag trigger callbacks (V19+).
 * Contains the tag value, quality, timestamp, and change indicator.
 */
interface TriggerTagData {
  /**
   * The current value of the tag.
   */
  Value: any;
  /**
   * The quality code of the tag value.
   */
  QualityCode: number;
  /**
  ...
  TimeStamp: number | string;
  /**
   * Indicates whether the tag value has changed since the last trigger.
   */
  HasChanged: boolean;
}
...
/**
 * Communication interface for system functions (V19+).
 * Provides access to communication-related system functions.
 */
interface HMICommunication {
  /**
   * System functions for communication operations.
   */
  SysFct: HMICommunicationSysFct;
}

/**
 * System functions available through HMIRuntime.Communication (V19+).
 */
interface HMICommunicationSysFct {
  /**
   * Sends an email via the configured mail server (V21+).
   *
   ...
   * @param attachments Optional file path(s) for attachments
   *
   * @returns ErrorCode
   */
  SendEmail(to: string, subject: string, body: string, cc?: string, bcc?: string, attachments?: string): number;
}
...
/**
 * User management system functions (V21+).
 */
interface HMIUserManagement {
  /**
   * System functions for user administration.
   */
  SysFct: HMIUserManagementSysFct;
}

/**
 * System functions available through UserManagement (V21+).
 */
interface HMIUserManagementSysFct {
  /**
   * Exports the current user administration data to a file (V21+).
   *
   * @param filePath Path where the export file will be written
   *
   * @returns ErrorCode
   */
  ExportUserAdministration(filePath: string): number;
  /**
   * Imports user administration data from a file (V21+).
   *
   * @param filePath Path to the import file
   *
   * @returns ErrorCode
   */
  ImportUserAdministration(filePath: string): number;
}

declare var HMIRuntime: HMIRuntime;
declare var UserManagement: HMIUserManagement;
